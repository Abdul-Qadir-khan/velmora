...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  // 🔥 Only 404 if ZERO products AND slug not in valid list
  const validSlugs = ['mens', 'womens', 't-shirts', 'jeans', 'shirts', 'jackets', 'hoodies'];
  if (products.length === 0 && !validSlugs.includes(slug)) {
    notFound();
  }
          ...
          <div className="grid lg:grid-cols-4 gap-8">
            <div className="lg:col-span-1 hidden lg:block sticky top-24">
              <Suspense fallback={<div className="h-96 bg-slate-100 rounded-2xl animate-pulse" />}>
                <ShopFilters filters={filters} params={{ category: slug, ...queryParams }} />
              </Suspense>
            </div>

            <div className="lg:col-span-3">
              <div className="mb-8">
                <h2 className="text-3xl lg:text-4xl font-light text-slate-900 mb-2">{categoryName}</h2>
              ...
              <Suspense fallback={
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {Array.from({ length: 12 }).map((_, i) => (
                    <div key={i} className="aspect-[3/4] bg-slate-100 rounded-2xl animate-pulse shadow-lg" />
                  ))}
                </div>
              }>
                <ClientProductGrid 
                  initialProducts={products} 
                  searchParams={{ category: slug, ...queryParams }}
                />
              </Suspense>
            </div>
          </div>
